...
func (s *Storage) CreateOrder(number string, userID int) error {
...
func (s *Storage) GetAllUserOrders(userID int) ([]*domain.Order, error) {
	...
	for rows.Next() {
		...
		if accrual.Valid {
			accrualValue := formatter.ConvertToCurrency(int(accrual.Int64))
...
func (s *Storage) UpdateOrderAccrualStatus(id int, status string, accrual *float64) error {
	tx, err := s.db.Begin()
	if err != nil {
